...
class Database : public SymbolConsumer {
 ...
 public:
  ...
  std::vector<clink::Result> find_symbol(const char *name) const;
  std::vector<clink::Result> find_definition(const char *name) const;
  std::vector<clink::Result> find_caller(const char *name) const;
  std::vector<clink::Result> find_call(const char *name) const;
  std::vector<std::string> find_file(const char *name) const;
  std::vector<clink::Result> find_includer(const char *name) const;

  // Give callers the convenience of passing a string instead.
  std::vector<clink::Result> find_symbol(const std::string &name) const {
    return find_symbol(name.c_str());
  }
  std::vector<clink::Result> find_definition(const std::string &name) const {
    return find_definition(name.c_str());
  }
  std::vector<clink::Result> find_caller(const std::string &name) const {
    return find_caller(name.c_str());
  }
  std::vector<clink::Result> find_call(const std::string &name) const {
    return find_call(name.c_str());
  }
  std::vector<std::string> find_file(const std::string &name) const {
    return find_file(name.c_str());
  }
  std::vector<clink::Result> find_includer(const std::string &name) const {
