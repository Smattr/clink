        ...
        switch (token.category) {
            ...
            case ASM_IDENTIFIER:
                if (state == IDLE || state == DEFINE) {
                    Symbol s(token.text, filename, ST_DEFINITION, line, column,
                        nullptr, get_context(line));
                    consumer.consume(s);
                    ...
                    Symbol s(token.text, filename, ST_FUNCTION_CALL, line,
                        column, last_defn == "" ? nullptr : last_defn.c_str(),
                        get_context(line));
                    consumer.consume(s);
                    state = IGNORING;
                } else if (state == HASH) {
            ...
            case ASM_STRING:
                if (state == INCLUDE) {
                    Symbol s(token.text, filename, ST_INCLUDE, line, column,
                        nullptr, get_context(line));
                    consumer.consume(s);
                }
                state = IGNORING;
                /* Account for the quotes that are not included in the contents
                 * of this token.
                 */
                column += 2;
                break;
