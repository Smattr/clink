...
int clink_db_find_definition(clink_db_t *db, const char *name,
  ...
  static const char QUERY[] =
      "select symbols.path, symbols.line, symbols.col, "
      ...
      "symbols.name = @name and symbols.category = @category order by "
  ...
  // save the name of the definition in the state for later symbol construction
  s->name = strdup(name);
  ...
  if (UNLIKELY((rc = sql_bind_text(s->stmt, 1, s->name))))
    goto done;
  if (UNLIKELY((rc = sql_bind_int(s->stmt, 2, CLINK_DEFINITION))))
    goto done;
